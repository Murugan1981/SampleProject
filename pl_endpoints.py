...
async def extract_endpoints(page):
    ...
    for block in get_blocks:
        try:
            # Expand GET block
            await block.query_selector(SUMMARY_SELECTOR)
            await block.click()
            await page.wait_for_timeout(300)

            # Endpoint path
            path_el = await block.query_selector(PATH_SELECTOR)
            endpoint = await path_el.inner_text() if path_el else ""

            # Tag
            tag_el = await block.query_selector("xpath=ancestor::div[contains(@class,'opblock-tag-section')]//h3")
            ...
            for row in rows:
                ...
                if name_el:
                    ...
                    if enum_el:
                        values = (await enum_el.inner_text()).replace("Available values:", "").strip()
                    else:
                        values = ""
            ...
            print(f"Skipping endpoint due to error: {e}")

    return results


async def process_env(env_name, url):
...
async def main():
    ...
    source_df = await process_env("SOURCE", SOURCE_DS)
    target_df = await process_env("TARGET", TARGET_DS)
    ...
    print(f"Endpoints extracted successfully → {OUTPUT_FILE}")
